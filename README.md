...
- **车库** [玩家名称] [等级] [类型]：查询玩家车库的坦克战绩，并可按等级、类型筛选
  ...
  - 示例：`/车库`、`/车库 10级 重坦` 或 `/车库 玩家名称 8级 中坦`
- **单车** [玩家名称] [坦克名称]：查询玩家指定坦克的场次、胜率、WN8、场均伤害和环数
  - 示例：`/单车 59式` 或 `/单车 玩家名称 查狄伦 25t`
- **环线** / **标伤** [坦克名称]：查询坦克一环/二环/三环标伤阈值（近7天）
  - 支持简称模糊查询；匹配多辆坦克时会返回全部结果
  - 查询结果以图片返回
  - 示例：`/环线 59`、`/环线 野牛` 或 `标伤 鞭蛇`

以上三个命令的数据来自坦克营地（WoT Box）接口，优先使用绑定关系中的账号 ID；
未绑定或指定其他玩家时，会通过坦克营地搜索玩家。

### 坦克百科与对比

- **坦克** [坦克名称]：查询坦克的火力、机动、视野等百科属性
  - 示例：`/坦克 59式`
