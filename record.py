...
from data.plugins.astrbot_plugin_wot.config.config import wot_box_detail_record_config, wot_box_records_config
from data.plugins.astrbot_plugin_wot.model.player_info import RecordsBasic, RecordsDetail, Tank, FinalSummary, TankSummary, OverallSummary
from data.plugins.astrbot_plugin_wot.util.utils import get_tank_info_by_name
from data.plugins.astrbot_plugin_wot.util.request_utils import send_get_request


def get_arena_page(player_name:str,page_num:int) ->list[RecordsBasic]:
    """获取战绩列表"""
...
def get_arena_list_by_days(player_name: str, days: int = 1) -> list[RecordsBasic]:
    ...
    while not stop_flag:
        try:
            ...
            page_records = get_arena_page(player_name, page)
            ...
            for record in page_records:
                ...
                try:
                    ...
                    print(f"第{page}页记录{record.arena_id}时间戳无效，跳过")
                    continue
...
def calculate_overall_summary(detail_record_list:list[RecordsDetail]) -> OverallSummary:
    # 1. 初始化累加变量
...
def calculate_tank_summary(detail_record_list:list[RecordsDetail]) -> list[TankSummary]:
...
def get_final_summary(detail_record_list:list[RecordsDetail]) -> FinalSummary:
    return FinalSummary(
        ...
        tank_summary=calculate_tank_summary(detail_record_list),
    )
