...
def get_player_box_stats(player_name: str) ->tuple[PlayerStats,list[FrequentTank]]:
    ...
    # 使用换行符拼接
    stats_data['comment'] = "\n".join(texts)
    stats_data['radar_data'] = _get_radar_data(res.text)

    player_stats = PlayerStats(**stats_data)
    frequent_tank_list = _get_frequent_tank_list(soup.find_all('div',class_='user-tank__pop'))
    return player_stats,frequent_tank_list

def _get_frequent_tank_list(tank_list_div) ->list[FrequentTank]:
    """获取常用坦克数据"""
    ...
    for pop in tank_list_div:
        ...
        frequent_tanks.append(tank_info)
    return frequent_tanks


def _get_radar_data(html_content):
    """
    获取雷达图数据
