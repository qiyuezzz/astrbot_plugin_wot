...
from data.plugins.astrbot_plugin_wot.src.infrastructure.parsers.number_utils import clean_number
from data.plugins.astrbot_plugin_wot.src.infrastructure.storage.tank_repo import get_tank_info_by_name


class WotBoxStatsParser:
    """Parse WotBox HTML into domain models."""

    def parse_player_stats(self, raw_html: str, player_name: str) -> tuple[PlayerStats, list[FrequentTank]]:
    ...
    def _parse_charts_data(soup: BeautifulSoup, stats_data: dict) -> None:
        ...
        if win_data:
            ...
            if total_tag:
                stats_data["total_count"] = clean_number(total_tag.get_text(), to_int=True)
            ...
            if result_tag:
                ...
                stats_data["win_count"] = clean_number(win_span.get_text(), to_int=True) if win_span else ""
                stats_data["lose_count"] = clean_number(fail_span.get_text(), to_int=True) if fail_span else ""
    ...
    def _parse_frequent_tanks(soup: BeautifulSoup) -> list[FrequentTank]:
        ...
        for pop in tank_list_div:
            ...
            win_rate = header.find("span", class_="win num").get_text(strip=True) if header.find(
                "span", class_="win num"
            ) else "0"
