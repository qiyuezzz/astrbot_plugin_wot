...
from data.plugins.astrbot_plugin_wot.api import get_all_tank_info, get_record_today, get_record_two_days, \
    get_record_three_days, get_record_yesterday
...
class MyPlugin(Star):
    ...
    async def get_all(self, event: AstrMessageEvent):
        """监听所有"""
    ...
    async def get_yesterday_performance(self, event: AstrMessageEvent):
        ...
        message_chain = event.get_messages()
        logger.info(message_chain)
        get_record_yesterday(send_id,'昨日战绩')
        chain = [
            Comp.At(qq=send_id),  # At 消息发送者
            Comp.Image.fromFileSystem(f"data/plugins/astrbot_plugin_wot/static/report/{send_id}.png"),  # 从本地文件目录发送图片
        ]
    ...
    async def get_two_days_performance(self, event: AstrMessageEvent):
        ...
        message_chain = event.get_messages()
        logger.info(message_chain)
        get_record_two_days(send_id,'两日战绩')
        chain = [
            Comp.At(qq=send_id),  # At 消息发送者
            Comp.Image.fromFileSystem(f"data/plugins/astrbot_plugin_wot/static/report/{send_id}.png"),  # 从本地文件目录发送图片
        ]
    ...
    async def get_three_days_performance(self, event: AstrMessageEvent):
        ...
        message_chain = event.get_messages()
        logger.info(message_chain)
        get_record_three_days(send_id,'三日战绩')
        chain = [
            Comp.At(qq=send_id),  # At 消息发送者
            Comp.Image.fromFileSystem(f"data/plugins/astrbot_plugin_wot/static/report/{send_id}.png"),  # 从本地文件目录发送图片
        ]
